...
const byTenant = (table, tenantId) => ddb.send(new QueryCommand({
    TableName: table,
    IndexName: 'tenantId-index',
    KeyConditionExpression: 'tenantId = :t',
    ExpressionAttributeValues: { ':t': tenantId },
}));
const byBroker = (table, brokerId, tenantId) => ddb.send(new QueryCommand({
    TableName: table,
...
export const handler = async (event) => {
    ...
    const fetch = adminView
        ? (t) => byTenant(t, tenantId)
        : (t) => byBroker(t, brokerId, tenantId);
    try {
