...
export const handler: APIGatewayProxyHandlerV2WithJWTAuthorizer = async (event) => {
  ...
  try {
    ...
    if (method === 'POST' && path === '/claims') {
      if (!isAdmin(groups)) return FORBIDDEN_ADMIN;
    ...
    if (method === 'GET' && path === '/claims') {
      if (!isAdmin(groups)) return FORBIDDEN;
    ...
    if (method === 'GET' && path === '/claims/my') {
      if (!isBroker(groups) && !isAdmin(groups)) return FORBIDDEN;
    ...
    if (method === 'GET' && path.startsWith('/claims/')) {
      ...
      if (!claim.Item) return NOT_FOUND;
      if (claim.Item.tenantId !== tenantId) return NOT_FOUND;
